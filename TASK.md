...
| `README.md` | How to start both applications from scratch (`git clone` → working screen). **«Ми буквально виконаємо ці кроки на чистій машині»** — *they will literally execute these steps on a clean machine.* | `docs/scoring-form/README.md`. The repository root `README.md` is upstream OHIF's and is left untouched for now — **a discoverable pointer from the root README to the task guide is required before submission** (`IMPLEMENTATION_PLAN.md` PR 7). |
