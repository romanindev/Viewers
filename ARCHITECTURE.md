...
> **Status.** Design document, partially implemented. As of GitHub PR #2 (`b2c5f4a4d`), the host app (`apps/host-app`) and the shared message contract (`packages/message-contract`) exist and are verified working. **PR #3 (`extensions/scoring-form-bridge`, the `VIEWER_READY` handshake) is merged to `master`** (`e2df3ff0e`) — see `docs/scoring-form/README.md` for the observed results. Tool activation (§6, §10.11) has an agreed design but is not implemented yet; implementation is targeted for PR #4; measurement correlation and totals (§7 onward) remain agreed design, not shipped code, until PR #5/6 land them.
>
...
Small explicit guards in the shared package, no schema library — the protocol has six message types. If it grows across teams, Zod or JSON-Schema-generated types become worthwhile; that is a documented future step, not a present need.
...
**Exclusive activation.** Only one row may be pending or armed. Activating row B while row A is pending (queued or armed) cancels A first, per the same still-queued-vs-already-dispatched rule above, so two rows can never both show `drawing`.
